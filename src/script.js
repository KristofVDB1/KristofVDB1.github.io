...
function setupCamera () {
  ...
  camera.position.set(-31, -100, -100)
  camera.far = 10000
...
function createTerrain () {
  new GLTFLoader().load('assets/images/island-big.glb', (res) => loadThing(res))
}
  ...
  if (o.isMesh) {
    ...
    for (let i = 0; i < cols.count; i += 3) {
      ...
      if (max <= p.water) setCol(cols, i, 0.0, 0.3, 1.0) // blue
        ...
        setCol(cols, i, 0.44, 0.7, 0.10) // green
      } else if (max <= p.rock) setCol(cols, i, 0.3, 0.3, 0.4) // gray
      else setCol(cols, i, 0.92, 0.98, 0.9) // white
    }
